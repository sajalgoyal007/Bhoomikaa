...
const LoginUserPage: React.FC = () => {
  ...
  const handleStart = () => {
    ...
    toast({
      title: "OTP Sent (Demo)",
      description: "OTP shown on screen for demo purposes.",
    });
  };
  ...
  const handleVerify = async () => {
    ...
    if (ok) {
      ...
      localStorage.setItem("bhoomika_user_auth", "true");

      toast({
        title: "Login Successful",
        description: "Welcome to Bhoomikaa!",
      });
  ...
  return (
    <div className="min-h-screen tribal-pattern flex items-center justify-center px-4">
      <Card className="w-full max-w-md vintage-border">
        <CardHeader>
          <CardTitle className="text-2xl text-center">
            Citizen Login (Aadhaar)
          </CardTitle>
        </CardHeader>
          ...
          {step === 2 && (
            <>
              <div className="text-center">
                ...
                <p className="font-mono text-lg font-bold">{simOtp}</p>
              </div>
